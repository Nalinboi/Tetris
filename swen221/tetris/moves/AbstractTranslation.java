...
public abstract class AbstractTranslation extends AbstractMove implements Move {
	...
	public boolean isValid(Board board) {
		...
		if(tetromino != null) { 
			if(dx == -1 && tetromino.canGoLeft(board)) {
				return true;
			}
			else if(dx == -1 && tetromino.canGoLeft(board) == false){
				return false;
			}
			
			if(dx == 1 && tetromino.canGoRight(board)) {
				return true;
			}
			else if(dx == 1 && tetromino.canGoRight(board) == false){
				return false;
			}
			
			if(dy == -1 && tetromino.canGoDown(board)) {
				return true;
			}
			else if(dx == -1 && tetromino.canGoDown(board) == false){
				return false;
			}
			
			if(board.canPlaceTetromino(tetromino) && tetromino.getBoundingBox().getMinY()>0) {
				return true;
			}
			else {
				return false;
